...
 [FAQ](..) > [Using Apache ActiveMQ Classic](.) > [Can two brokers share the same database](can-two-brokers-share-the-same-database)
...
Also if you want to only have one database but many possible brokers (for HA) then just use [JDBC Master Slave](../../../features/clustering/masterslave/jdbc-master-slave)
