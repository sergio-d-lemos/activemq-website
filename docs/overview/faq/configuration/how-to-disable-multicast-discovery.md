...
 [FAQ](..) > [Configuration](.) > [How to disable multicast discovery](how-to-disable-multicast-discovery)


By default, the [ActiveMQ Classic xml configuration](../../../using-activemq-classic/xml-configuration) includes the [multicast discovery](../../../features/discovery) mechanism. The tcp transport connector advertises its self using multicast and a multicast network connector is configured to listen to the same address. In this way, all brokers that share the default multicast address will automatically network with each other.  
