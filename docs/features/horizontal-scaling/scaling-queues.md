...
With the default configuration, ActiveMQ Classic is configured to use a dispatch thread per Queue - you can use set the optimizedDispatch property on the destination policy entry - see [configuring Queues](../destination-features/per-destination-policies).
...
Reduce the memory used per thread - see [reducing memory consumption](../../overview/faq/errors/Exceptions/javalangoutofmemory)
